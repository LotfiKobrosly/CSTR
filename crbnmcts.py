...
def crbnmcts(environment: EnvironmentWrapper, level: int = 1, bandwidth: int = 20, action=None):
    if level == 0:
        environment.step(action)
        return list(), environment.get_last_reward()
        ...
        while not environment.is_final():
            ...
            while len(actions_list) < bandwidth:
                ...
                if not np.any([np.all(new_action == element) for element in actions_list]):
